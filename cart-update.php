...
if ($sp_id <= 0) { echo json_encode(['status'=>'error','message'=>'Thiếu id'], JSON_UNESCAPED_UNICODE); exit; }
if ($qty < 1) $qty = 1;

$stmt = $conn->prepare("UPDATE giohang SET soluong=? WHERE khachhang_id=? AND sanpham_id=?");
$stmt->bind_param("iii", $qty, $user_id, $sp_id);
$stmt->execute();
